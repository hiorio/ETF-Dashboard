...
section("2c. 네이버 금융 - 분배금 이력 (ETF 분배금 탭)")
try:
    import pandas as pd
    from io import StringIO
    headers = {"User-Agent": "Mozilla/5.0"}
    # 네이버 ETF 분배금 이력 URL
    url = f"https://finance.naver.com/item/coinfo.naver?code={KR_CODE}&target=etfdiv"
    res = requests.get(url, headers=headers, timeout=10)
    soup = BeautifulSoup(res.text, "lxml")
    tables = pd.read_html(StringIO(res.text))
    if tables:
        ok(f"테이블 {len(tables)}개 발견")
...
[OK] 표시된 소스를 기반으로 collector.py를 작성합니다.
""")
